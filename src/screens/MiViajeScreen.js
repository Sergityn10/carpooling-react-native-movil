// YouConnext - MiViajeScreen (vista de pasajero)
...
import { TripMapPreview } from "../components";
...
const MiViajeScreen = ({ route, navigation }) => {
  const { viaje: viajeInicial, id: idDirecto } = route.params || {};
  const targetId = idDirecto || viajeInicial?.id;
  const { user } = useUser();
  const { trackingActivo, iniciarTrackingPasajero, detenerTrackingPasajero } =
    useViaje();
  ...
  const cargarDetalleViaje = async () => {
    ...
    try {
      ...
      console.log(
        "[MiViaje] /completo response completa:",
      ...
      console.log(
        "[MiViaje] pasajeros desde /completo:",
      ...
      if (!pasajerosList || pasajerosList.length === 0) {
        try {
          ...
          console.log(
            "[MiViaje] fallback reservaService response:",
            JSON.stringify(reservasData, null, 2),
          );
          pasajerosList = reservasData?.pasajerosList || reservasData || [];
          console.log(
            "[MiViaje] pasajeros tras fallback:",
            JSON.stringify(pasajerosList, null, 2),
          );
        } catch (e) {
          console.log("[MiViaje] Fallback reservas fallido:", e);
        }
      }
      ...
      if (vehiculoId) {
        try {
          ...
          console.log("[MiViaje] No se pudo cargar el vehículo:", e?.message);
          setVehiculo(null);
        }
      }
      ...
      if (user?.id) {
        ...
        console.log(
          "[MiViaje] reserva del usuario actual:",
          ...
          try {
            ...
            if (
              ...
              try {
                ...
                console.warn(
                  "[MiViaje] No se pudo reanudar tracking:",
                  e?.message,
                );
              }
            }
          } catch (e) {
            console.log("[MiViaje] No se pudo cargar estado del pasajero:", e);
          }
        }
      }
    } catch (error) {
    ...
    return (
      <SafeAreaView style={styles.container} edges={["top"]}>
        <View style={styles.header}>
          ...
          <Text style={styles.headerTitle}>Mi viaje</Text>
        </View>
    ...
    <SafeAreaView style={styles.container} edges={["top"]}>
      ...
      >
        ...
        {reservaExistente && (
          <View style={styles.reservaCard}>
            {reservaExistente.status === "completed" ? (
              ...
              <AlertCircle size={22} color={COLORS.warning} strokeWidth={2.2} />
            ) : (
              <XCircle size={22} color={COLORS.gray400} strokeWidth={2.2} />
            )}
        ...
        <View style={styles.actions}>
                ...
                {reserving ? (
                  ...
                  <>
                    <CheckCircle2
                      ...
                      strokeWidth={2.2}
                    />
                    <Text style={styles.primaryBtnText}>Ya me ha recogido</Text>
                  </>
                )}
              </TouchableOpacity>
            )}
            ...
            !estadoPasajero?.pasajero?.en_destino && (
              ...
              >
                {reserving ? (
                  ...
                  <>
                    <MapPin size={18} color={COLORS.white} strokeWidth={2.2} />
          ...
          {(llegadaRegistrada || estadoPasajero?.pasajero?.en_destino) && (
            <View style={styles.confirmCard}>
            ...
            reservaExistente.status !== "canceled" && (
              ...
              >
                {reserving ? (
                  ...
                  <Text style={styles.dangerBtnText}>Cancelar reserva</Text>
                )}
              </TouchableOpacity>
            )}
        </View>
